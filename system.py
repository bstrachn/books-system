...
def add_book():
  conn = mysql.connector.connect(
       host='localhost', database='textbook', user='root', password='inuyasha12')
  cursor = conn.cursor()

  title = input('Enter Book Title :')
  author = input('Enter Book Author : ')
  course = input('Enter Book course : ')
  pages = input('Enter Book Pages : ')
  price = input('Enter Book Price : ')
  edition = input('Enter Book Edition : ')
  copies  = int(input('Enter copies : '))

  sql = 'insert into book(title,author,price,pages,course,edition,status) values ( "' + \
       title + '","' + author+'",'+price+','+pages+',"'+course+'","'+edition+'","available");'
...
def report_member_list():
    conn = mysql.connector.connect(
        host='localhost', database='Books', user='root', password='inuyasha12')
    cursor = conn.cursor()
